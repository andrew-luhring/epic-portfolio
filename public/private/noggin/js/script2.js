...
$(document).ready(function() {
	...
	var keyA;
	var getstuff = document.getElementById("getStuff").value;
	function process() {
		if (request.readyState === 0 || request.readyState === 4) {
			keyA = "fa96520990157f8a3077251dabf16267";
			request.open("GET", "http://recruiting.nogginlabs.com/web-service/api/get_task_list.php?key=" + keyA, true);
	...
	function handleServerResponse() {
		if (request.readyState === 4 && request.status === 200) {
			var theResponse = request.response;
			//var theResponse = request.responseText;
			var responseString = JSON.parse(theResponse);
			
			var output;
			
			for (key in responseString ){
				output = responseString[key];
			}
			console.log(output.length);
			for(var i = 0; i < output.length; i++ ){
			
				$('#responded').text(output);
				
			}
			var responseString = JSON.stringify(theResponse, '', 2);
		} else {
			$("responded").text(request.status);
		}
    ...
    if (!(!!(audioTag.canPlayType) && ("no" != audioTag.canPlayType("audio/mpeg")) && ("" != audioTag.canPlayType("audio/mpeg"))&& ("no" != audioTag.canPlayType("audio/ogg")) )) {
        AudioPlayer.embed("audioplayer", {soundFile: "audio.mp3"});
    }
//md5 hash fa96520990157f8a3077251dabf16267
