...
jQuery(document).ready(function($) {
	...
	function clickaBox() {
		if ($(".workTypes").not(".workTypesSelected")) {
			fadeThemOut(worktypes);
			$('#hint').css("visibility", "visible").text("(if you want to see my work, just click one of these buttons.)").show("fade", 500);
			timeoutId = window.setTimeout(joke, 90000);
		}
	}

	function joke() {
		$('#hint').text("I mean...if you don't want to you don't have to- just keep doing what you're doing.").show("fade", 500);
		timeoutId = window.setTimeout(joke2, 10000);
	}

	function joke2() {
		$('#hint').text("(which is nothing).").show("fade", 500);
		timeoutId = window.setTimeout(joke3, 5000);
	}

	function joke3() {
		$('#hint').text("...so...how are things?").show("fade", 500);
		timeoutId = window.setTimeout(joke4, 10000);
	}

	function joke4() {
		$('#hint').text("did you really respond to that question in your head?").show("fade", 500);
		timeoutId = window.setTimeout(joke5, 10000);
	}

	function joke5() {
		$('#hint').text("if you did, just a heads up- next time i ask you a question, you don't have to answer it...i'm not real").show("fade", 500);
	...
	function joke8() {
		$('#hint').text("have you ever been on a website while it's having an existential crisis?").show("fade", 500);
		timeoutId = window.setTimeout(joke9, 10000);
	}

	function joke9() {
		$('#hint').text("now you're just being cruel. i'm just trying to do my job.").show("fade", 500);
		timeoutId = window.setTimeout(joke10, 10000);
	}

	function joke10() {
		$('#hint').text("come on. do me a solid and click one of the buttons. i'm getting anxious because you're just sitting there.").show("fade", 500);
		timeoutId = window.setTimeout(joke11, 10000);
	}

	function joke11() {
		$('#hint').text("ok, i can't take it anymore. i refuse to play your mind games, visitor. here's some content.").show("fade", 500);
	...
	function jokecontd() {
		$('#galleryDiv').text("BAM. now, in case you want to see some stuff, there's a box here to put that stuff in. (no pressure).");
		timeoutId = window.setTimeout(jokecontd1, 10000);
	}

	function jokecontd1() {
		$('#galleryDiv').text("hey. thanks for letting me do stuff for you.");
		timeoutId = window.setTimeout(jokecontd2, 10000);
	}

	function jokecontd2() {
		$('#galleryDiv').text("oh. and server wanted me to tell you he said it has been a pleasure to almost serve you, so far.");
...
jQuery(document).ready(function($){
	...
	var crawlHeight = $(".scrolling").height();
	setTimeout(crawl, 2000);
	function crawl(){
		$("#time").removeClass("hide");
		$("#smaller").delay(2400).animate({
			"opacity": 1
			}, "slow", function(){
				$("#smaller, #time").delay(2000).animate({
					"opacity" : 0
				}, "slow", function(){
					$("#epic").delay(2000).removeClass("hide");
					$(".scrolling").delay(1000).removeClass("hide-dn");
					var a = $(".scrolling").offset().top;
					var ah = $(".scrolling").height();
					a = a + ah;
					console.log(a);
					$("#epic").transition({
						top : -(254) + "px",
	...
	$("#quit").click(function(){
		$("#modal, #time, #smaller, .scrolling, #epic, #quit").addClass("hide").stop(true,true);
	});
});
