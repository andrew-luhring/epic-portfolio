...
jQuery(document).ready(function($) {
		...
		function showGallery(){
			...
			jQuery.each($('img.gallery, #galleryDiv a'), function() {
				$(this).show("fold", "500").switchClass("hide", "unhide", 1500);
			});
		...
		$('.workTypes').click(function() {
				...
				if ($(this).is('#other')) {
					...
					$('#galleryDiv').append(
					'<p id="temp" style="text-align:center">Click the pic to play with the code</p> <p id="clickToClose">Show</p>')
					.show(500);
					...
					$('#galleryDiv').append(
						'<p id="temp">My most important portfolio piece is everything on bisforbounce- I wrote all of the code on here except the plugins (shadowbox, modernizr, animate shadow css).</p><p id="clickToClose">Show</p>')
					.show(500);
